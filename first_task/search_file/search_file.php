...
#Parse filename passed from CLI into array of ips
$raw_file = file_get_contents($filename);
$ips = get_ips_from_file($raw_file);

search_file($ips,$wanted_ip,$raw_ip);

/**
 * get_ips_from_file
 * 
 * Parses text file into array.
 *
 * Function explodes rows from text file and fetches ip and mask from string.
 *
 * @param $raw_file
 * @return array  array of ips from file
 *
 */
function get_ips_from_file ($raw_file) {
	$raw_ips = array();
	$parsed_rows = explode("\n", $raw_file);
 ...
 * search_file
 * 
 * Searches file for ip and outputs the result of search.
 *
 * Accepts array of ips which is fetched from text file, wanted ip and ip from CLI.
 * Goes trough every ip from file, gets the range of ips based on ip and mask and check if
 * ip passed from CLI is in that range of ips. Based on check results it outputs messages.
 *
 * @param $ip_list list of ips from file
 * @param $wanted_ip  ip passed from CLI in long format
 * @param $raw_ip ip passed from CLI
 *
 */
function search_file($ip_list,$wanted_ip,$raw_ip) {
	...
	print_r("Sorry " . $raw_ip . " does not match any ip in file \n");
}
